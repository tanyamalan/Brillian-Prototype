"use client";
require("../../_virtual/_rolldown/runtime.cjs");
const require_create_split_props = require("../../utils/create-split-props.cjs");
const require_factory = require("../factory.cjs");
const require_use_pin_input_context = require("./use-pin-input-context.cjs");
const require_use_pin_input = require("./use-pin-input.cjs");
let react = require("react");
let _zag_js_react = require("@zag-js/react");
let react_jsx_runtime = require("react/jsx-runtime");
//#region src/components/pin-input/pin-input-root.tsx
var splitRootProps = require_create_split_props.createSplitProps();
var PinInputRoot = (0, react.forwardRef)((props, ref) => {
	const [usePinInputProps, localProps] = splitRootProps(props, [
		"autoFocus",
		"autoSubmit",
		"blurOnComplete",
		"count",
		"defaultValue",
		"disabled",
		"form",
		"id",
		"ids",
		"invalid",
		"mask",
		"name",
		"onValueChange",
		"onValueComplete",
		"onValueInvalid",
		"otp",
		"pattern",
		"placeholder",
		"readOnly",
		"required",
		"sanitizeValue",
		"selectOnFocus",
		"translations",
		"type",
		"value"
	]);
	const pinInput = require_use_pin_input.usePinInput(usePinInputProps);
	const mergedProps = (0, _zag_js_react.mergeProps)(pinInput.getRootProps(), localProps);
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(require_use_pin_input_context.PinInputProvider, {
		value: pinInput,
		children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(require_factory.ark.div, {
			...mergedProps,
			ref
		})
	});
});
PinInputRoot.displayName = "PinInputRoot";
//#endregion
exports.PinInputRoot = PinInputRoot;
