"use client";
require("../../_virtual/_rolldown/runtime.cjs");
const require_create_split_props = require("../../utils/create-split-props.cjs");
const require_factory = require("../factory.cjs");
const require_use_file_upload_context = require("./use-file-upload-context.cjs");
const require_use_file_upload = require("./use-file-upload.cjs");
let react = require("react");
let _zag_js_react = require("@zag-js/react");
let react_jsx_runtime = require("react/jsx-runtime");
//#region src/components/file-upload/file-upload-root.tsx
var splitRootProps = require_create_split_props.createSplitProps();
var FileUploadRoot = (0, react.forwardRef)((props, ref) => {
	const [useFileUploadProps, localProps] = splitRootProps(props, [
		"accept",
		"acceptedFiles",
		"allowDrop",
		"capture",
		"defaultAcceptedFiles",
		"directory",
		"disabled",
		"id",
		"ids",
		"invalid",
		"locale",
		"maxFiles",
		"maxFileSize",
		"minFileSize",
		"name",
		"onFileAccept",
		"onFileChange",
		"onFileReject",
		"preventDocumentDrop",
		"readOnly",
		"required",
		"translations",
		"transformFiles",
		"validate"
	]);
	const fileUpload = require_use_file_upload.useFileUpload(useFileUploadProps);
	const mergedProps = (0, _zag_js_react.mergeProps)(fileUpload.getRootProps(), localProps);
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(require_use_file_upload_context.FileUploadProvider, {
		value: fileUpload,
		children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(require_factory.ark.div, {
			...mergedProps,
			ref
		})
	});
});
FileUploadRoot.displayName = "FileUploadRoot";
//#endregion
exports.FileUploadRoot = FileUploadRoot;
