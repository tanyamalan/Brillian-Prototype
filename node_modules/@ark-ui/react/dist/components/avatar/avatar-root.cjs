"use client";
require("../../_virtual/_rolldown/runtime.cjs");
const require_create_split_props = require("../../utils/create-split-props.cjs");
const require_factory = require("../factory.cjs");
const require_use_avatar_context = require("./use-avatar-context.cjs");
const require_use_avatar = require("./use-avatar.cjs");
let react = require("react");
let _zag_js_react = require("@zag-js/react");
let react_jsx_runtime = require("react/jsx-runtime");
//#region src/components/avatar/avatar-root.tsx
var splitRootProps = require_create_split_props.createSplitProps();
var AvatarRoot = (0, react.forwardRef)((props, ref) => {
	const [useAvatarProps, localProps] = splitRootProps(props, [
		"id",
		"ids",
		"onStatusChange"
	]);
	const avatar = require_use_avatar.useAvatar(useAvatarProps);
	const mergedProps = (0, _zag_js_react.mergeProps)(avatar.getRootProps(), localProps);
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(require_use_avatar_context.AvatarProvider, {
		value: avatar,
		children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(require_factory.ark.div, {
			...mergedProps,
			ref
		})
	});
});
AvatarRoot.displayName = "AvatarRoot";
//#endregion
exports.AvatarRoot = AvatarRoot;
